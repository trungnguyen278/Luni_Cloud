/* ============================================================
   mock/data.ts — seed data shaped to the real Luni Cloud API.
   Ported from web-data.jsx. Used as demo data for sections whose
   backend endpoint does not exist yet (admin fleet, emotion dist),
   and as initial/fallback shapes elsewhere. Gated via lib/flags.ts.
   ============================================================ */
...
export const FLEET_STATUS: Record<FleetStatus, StatusMeta> = {
...
export const CHANNEL: Record<string, ChannelMeta> = {
  stable: { c: '#7BE88E', label: 'stable' },
  beta: { c: '#FFD166', label: 'beta' },
};

/* server + device logs — GET /admin/logs/* */
export const LOG_LV: Record<string, LogLevelMeta> = {
...
/* overview stats */
export const STAT_7D = [128, 156, 142, 198, 174, 220, 261];
export const STAT_LABELS = ['T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'CN'];
export const EMOTION_DIST: EmotionDistRow[] = [
...
export const MY_DEVICES: MyDevice[] = [
...
export const USER_7D = [6, 11, 8, 14, 9, 17, 13];
export const USER_BATTERY = [92, 88, 80, 71, 64, 84, 84];
